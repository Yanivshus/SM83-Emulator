...
pub mod cartridge {
    ...
    impl fmt::Display for CartridgeErr {
        fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
            match self {
                Self::NintendoLogoDoesntExists => write!(f, "The Nintendo logo doesn't match to the original one, Locking cpu..."),
                Self::FileReadingErr(err) => write!(f, "io problem {err:?} locking cpu..."),
                Self::WrongChecksum => write!(f, "Problem while calculating checksum, locking cpu..."),
            }
        }
    }

    pub fn get_cartridge_buffer(file_name: &String) -> Result<Vec<u8>, CartridgeErr> {
        use GbFileLocations::*;
