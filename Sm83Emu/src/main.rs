...
fn compute_checksum(barr: &[u8], checksum: u8) -> bool {
    ...
    for i  in GbFileLocations::TitleS as usize..GbFileLocations::HeaderCheckSum as usize  {
        comp_checksum = comp_checksum - barr[i] -1;
    }
...
impl Mmu {
    fn new(file_name: &String) -> Result<Self, MmuError> {
        ...
        let checksum: u8 = gb_file[GbFileLocations::HeaderCheckSum as usize]; // get checksum by the byte
        if !compute_checksum(&gb_file[0..(GbFileLocations::HeaderCheckSum as usize + 1)], checksum) {
