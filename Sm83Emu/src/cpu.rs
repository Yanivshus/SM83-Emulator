...
impl Cpu {
    ...
    pub fn execute_instruction(self: &mut Self, instruction: &Instruction) {
        match instruction {
            ...
            Instruction::INCBC => {
                self.registers
                    .set_bc(self.registers.get_bc().overflowing_add(1).0);
            }
            Instruction::INCB => {
            ...
            Instruction::LDa16SP(val) => {
                ...
                if (0xA000..=0xDFFF).contains(val) {
                    self.mmu
                        .write_byte(*val, ((self.registers.sp) & 0xFF) as u8);
                    self.mmu
            ...
            Instruction::ADDHLBC => {
                let half_carry =
                    ((self.registers.get_hl() & 0xF) + (self.registers.get_bc() & 0xF)) > 0xF;
...
mod tests {
    ...
    #[test]
    fn test_rlca() {
        let mut cpu = Cpu::new("/home/kaish/Downloads/Calc.gb");
